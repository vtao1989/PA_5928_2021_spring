...
We could add more attributes as aesthetic elements in the plot. For example, we could use color to indicate the value of ``EDUC`` by adding ``col = EDUC``. (``col``, ``color``, and ``colour`` all work here.)
...
Here, ``EDUC`` is a continuous variable, so ``ggplot2`` uses the darkness of color to indicate the value. However, if we use a categorical variable or a factor (e.g., binomial variable), ``ggplot2`` will use different colors to show different levels.
...
Besides ``color``, there are several other parameters to show various aesthetics of the plots. There are differences between continuous variable and categorical variable. Furthermore, if you want to map variables in those aesthetics, you have to do it in the ``aes()`` function.
...
```{r}
ggplot(minneaplis_race, aes(x = '', y = count, fill = factor(RACE))) +
...
```{r}
ggplot(minneapolis_2017, aes(x = factor(SEX), y = INCTOT)) + 
  geom_boxplot() +
  coord_flip()
```
...
```{r}
ggplot(minneapolis_2017, aes(factor(EDUC))) +
...
```{r}
p <- ggplot(minneapolis_emp, aes(x = AGE, y = INCTOT, color = factor(EMPSTAT))) +
  geom_point() +
  labs(
    x = 'Age',
    y = 'Personal income (dollars)',
